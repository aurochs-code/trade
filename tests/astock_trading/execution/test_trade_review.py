...
def test_review_due_trade_records_mfe_mae_and_hypothesis_validation(tmp_path):
...
def test_review_due_trade_reports_missing_market_bars_without_recording(tmp_path):
    db_path = tmp_path / "review.db"
    init_db(db_path)
    conn = connect(db_path)
    store = EventStore(conn)
