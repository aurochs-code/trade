    ...
    def agent_context(
        ...
        payload = {
            ...
            "recommended_commands": {
                ...
                "ops_watchdog": "atrade ops watchdog --json",
...
def _command_catalog_entries() -> list[dict[str, Any]]:
    return [
        ...
        _catalog_entry(
            id="diagnose_recommendations",
            title="推荐层诊断",
            argv=["atrade", "diagnose", "recommendations", "--json"],
            category="diagnostics",
            description="区分正式买入、试买观察、强观察和影子复盘推荐层；只读，不运行 pipeline。",
        ),
        _catalog_entry(
        ...
        _catalog_entry(
            id="ops_watchdog",
            title="系统化运维 watchdog",
            argv=["atrade", "ops", "watchdog", "--json"],
            category="operations",
            description="聚合调度、数据源、候选池和模拟承接断点；只读，不运行 pipeline、不下单。",
            options={
                "--include-account": {
                    "type": "flag",
                    "default": False,
                ...
                "--jobs-path": {"placeholder": "PATH", "required": False},
                "--env-file": {"placeholder": "PATH", "required": False},
                "--fail-on-critical": {"type": "flag", "default": False, "effect": "发现 critical 时退出码为 2"},
                "--json": {"required_for_automation": True},
            },
        ),
        _catalog_entry(
            id="ops_watchdog_supervise",
            title="监督运行运维 watchdog",
            argv=["atrade", "ops", "watchdog-supervise", "--json"],
            category="operations",
            description="父进程监督运行 notify ops-watchdog，超时会杀掉子进程；用于 launchd 独立巡检。",
            writes_state=True,
            writes_order=False,
            risk_level="notification_write",
            state_events=["ops_watchdog.state_file"],
            options={
                "--timeout-seconds": {"type": "int", "default": 60, "min": 5},
                "--json": {"required_for_automation": True},
            },
        ),
        _catalog_entry(
        ...
        _catalog_entry(
            id="data_sources_coverage",
            title="市场数据覆盖率",
            argv=["atrade", "data-sources", "coverage", "--json"],
            category="diagnostics",
            description="汇总 market_price_bars 的 source、复权口径、日期和股票覆盖；只读。",
            options={
                "--source": {"placeholder": "SOURCE", "required": False},
                "--adjustflag": {"placeholder": "ADJUSTFLAG", "required": False},
                "--start": {"placeholder": "YYYY-MM-DD", "required": False},
                "--end": {"placeholder": "YYYY-MM-DD", "required": False},
                "--json": {"required_for_automation": True},
            },
        ),
        _catalog_entry(
            id="data_sources_hydrate_market_bars",
            title="全市场日线补水",
            argv=["atrade", "data-sources", "hydrate-market-bars", "START", "END", "--source", "tushare", "--adjustflag", "3", "--write", "--json"],
            category="market_data",
            description="按交易日拉取 Tushare 全市场普通日线并写入 market_price_bars；不会下单。",
            writes_state=True,
            risk_level="market_data_write",
            state_events=["market_price_bars", "market_data_coverage"],
            arguments={
                "START": {"description": "补水开始日期 YYYY-MM-DD"},
                "END": {"description": "补水结束日期 YYYY-MM-DD"},
            },
            options={
                "--source": {"placeholder": "SOURCE", "default": "tushare"},
                "--adjustflag": {"placeholder": "ADJUSTFLAG", "default": "3"},
                "--limit-dates": {"type": "int", "required": False},
                "--write": {"type": "flag", "default": False, "writes_state": True},
                "--json": {"required_for_automation": True},
            },
        ),
        _catalog_entry(
        ...
        _catalog_entry(
            id="data_sources_replay_discovery",
            title="历史发现池回放",
            argv=["atrade", "data-sources", "replay-discovery", "START", "END", "--source", "tushare", "--adjustflag", "3", "--write", "--json"],
            category="research",
            description="从已落库 market_price_bars 重建每日历史发现池，并写入 pool-only 历史信号镜像；不会下单。",
            writes_state=True,
            writes_order=False,
            risk_level="state_write",
            state_events=["signal_history_snapshots"],
            arguments={
                "START": {"description": "回放开始日期 YYYY-MM-DD"},
                "END": {"description": "回放结束日期 YYYY-MM-DD"},
            },
            options={
                "--source": {"placeholder": "SOURCE", "default": "tushare"},
                "--adjustflag": {"placeholder": "ADJUSTFLAG", "default": "3"},
                "--lookback-days": {"type": "int", "default": 60},
                "--min-history-days": {"type": "int", "default": 20},
                "--min-avg-amount": {"type": "float", "default": 200000000.0},
                "--min-avg-amplitude": {"type": "float", "default": 3.0},
                "--min-price": {"type": "float", "default": 5.0},
                "--max-price": {"type": "float", "default": 200.0},
                "--limit": {"type": "int", "default": 600},
                "--write": {"type": "flag", "default": False, "writes_state": True},
                "--json": {"required_for_automation": True},
            },
        ),
        _catalog_entry(
            id="data_sources_index_discoveries",
            title="历史发现索引重建",
            argv=["atrade", "data-sources", "index-discoveries", "START", "END", "--write", "--json"],
            category="research",
            description="从 signal_history_snapshots 重建 code/date 发现索引，供 reachable-only 回测快速查询；不会下单。",
            writes_state=True,
            writes_order=False,
            risk_level="state_write",
            state_events=["signal_history_discoveries"],
            arguments={
                "START": {"description": "索引开始日期 YYYY-MM-DD"},
                "END": {"description": "索引结束日期 YYYY-MM-DD"},
            },
            options={
                "--write": {"type": "flag", "default": False, "writes_state": True},
                "--json": {"required_for_automation": True},
        ...
        _catalog_entry(
            id="backtest",
            title="历史回测",
            argv=["atrade", "backtest", "CODES", "START", "END", "--history-mirror", "--json"],
            category="research",
            description="运行历史回测并输出指标、最近交易样本、完整权益曲线和完整 trade_log；默认不写状态。",
            arguments={
                "CODES": {"description": "逗号分隔股票代码"},
                "START": {"description": "回测开始日期 YYYY-MM-DD"},
                "END": {"description": "回测结束日期 YYYY-MM-DD"},
            },
            options={
                "--preset": {"placeholder": "PRESET", "default": "攻_C_recovery_ma120_green_scale04"},
                "--initial-cash": {"type": "float", "default": 100000.0},
                "--history-mirror/--no-history-mirror": {"type": "flag", "default": True},
                "--use-stored-data": {"type": "flag", "default": False},
                "--hydrate-data": {"type": "flag", "default": False, "writes_state": True},
                "--commission-bps": {"type": "float", "default": 2.5, "min": 0.0},
                "--min-commission": {"type": "float", "default": 5.0, "min": 0.0},
                "--stamp-tax-bps": {"type": "float", "default": 5.0, "min": 0.0},
                "--transfer-fee-bps": {"type": "float", "default": 0.1, "min": 0.0},
                "--slippage-bps": {"type": "float", "default": 5.0, "min": 0.0},
                "--signal-slices": {
                    "type": "csv",
                    "default": "",
                    "description": "信号 Alpha 诊断切片白名单，如 volume_ratio_bucket,rsi_bucket",
                },
                "--record-run": {"type": "flag", "default": False, "writes_state": True},
                "--json": {"required_for_automation": True},
            },
        ),
        _catalog_entry(
            id="backtest_record_run",
            title="记录历史回测",
            argv=["atrade", "backtest", "CODES", "START", "END", "--history-mirror", "--record-run", "--json"],
            category="research",
            description="运行历史回测，并把本次运行、完整交易日志和权益曲线写入 MySQL；不下单。",
            writes_state=True,
            writes_order=False,
            risk_level="state_write",
            state_events=[
        ...
        _catalog_entry(
            id="backtest_runs",
            title="已记录回测",
            argv=["atrade", "backtest-runs", "--json"],
            category="research",
            description="读取最近已持久化的回测运行和指标摘要；只读。",
            options={
        ...
        _catalog_entry(
            id="replay_production",
            title="生产级历史重放",
            argv=["atrade", "replay-production", "START", "END", "--json"],
            category="research",
            description="按历史镜像、reachable-only、已落库数据、T+1、涨跌停和真实成本模型重放生产可达回测；只读。",
            arguments={
                "START": {"description": "重放开始日期 YYYY-MM-DD"},
                "END": {"description": "重放结束日期 YYYY-MM-DD"},
            },
            options={
                "--codes": {"placeholder": "CODES", "required": False},
                "--code-limit": {"type": "int", "default": 500, "min": 1},
                "--preset": {"placeholder": "PRESET", "default": "攻_C_recovery_ma120_green_scale04"},
                "--initial-cash": {"type": "float", "default": 100000.0},
                "--reachable-lookback-days": {"type": "int", "default": 5, "min": 0},
                "--financials/--skip-financials": {"type": "flag", "default": True},
                "--signal-alpha/--no-signal-alpha": {"type": "flag", "default": True},
                "--trade-output-limit": {"type": "int", "default": 50},
                "--progress-log/--quiet-progress": {"type": "flag", "default": False},
                "--json": {"required_for_automation": True},
            },
        ),
        _catalog_entry(
            id="backtest_batch",
            title="批量历史回测",
            argv=["atrade", "backtest-batch", "CODES", "START", "END", "--history-mirror", "--json"],
            category="research",
            description="大样本批量回测：批次隔离、超时拆分，并汇总信号 Alpha；默认不写状态。",
            arguments={
                "CODES": {"description": "逗号分隔股票代码"},
                "START": {"description": "回测开始日期 YYYY-MM-DD"},
                "END": {"description": "回测结束日期 YYYY-MM-DD"},
            ...
            options={
                "--preset": {"placeholder": "PRESET", "default": "攻_C_recovery_ma120_green_scale04"},
                "--initial-cash": {"type": "float", "default": 100000.0},
                "--batch-size": {"type": "int", "default": 8, "min": 1},
                "--batch-timeout": {"type": "float", "default": 240.0, "min": 1.0},
                "--history-mirror/--no-history-mirror": {"type": "flag", "default": True},
                "--use-stored-data": {"type": "flag", "default": False},
                "--hydrate-data": {"type": "flag", "default": False, "writes_state": True},
                "--use-market-bars": {"type": "flag", "default": False},
                "--hydrate-market-bars": {"type": "flag", "default": False, "writes_state": True},
                "--reachable-only/--no-reachable-only": {"type": "flag", "default": False},
                "--commission-bps": {"type": "float", "default": 2.5, "min": 0.0},
                "--min-commission": {"type": "float", "default": 5.0, "min": 0.0},
                "--stamp-tax-bps": {"type": "float", "default": 5.0, "min": 0.0},
                "--transfer-fee-bps": {"type": "float", "default": 0.1, "min": 0.0},
                ...
                "--signal-output-limit": {"type": "int", "default": 200, "min": 0},
                "--progress-log/--quiet-progress": {"type": "flag", "default": False},
                "--json": {"required_for_automation": True},
            },
        ),
        ...
        _catalog_entry(
            ...
            options={
                ...
                "--dry-run": {
                    "type": "flag",
                    "default": False,
                    "effect": "只演练刷新评分链路，不写候选池、报告或历史归档",
                },
                "--scoring-timeout-seconds": {
                    "type": "float",
                    "required": False,
                    "default": "strategy.screening.screener_scoring_timeout_seconds",
                },
                "--json": {"required_for_automation": True},
            },
        ),
        ...
        _catalog_entry(
            ...
            options={
                ...
                "--daily-atr-pct": {"type": "float", "default": 0.0},
                "--target-vol-pct": {"type": "float", "required": False},
                "--json": {"required_for_automation": True},
            },
        ),
        _catalog_entry(
        ...
        _catalog_entry(
            id="notify_ops_watchdog",
            title="推送运维 watchdog 状态变化",
            argv=["atrade", "notify", "ops-watchdog", "--json"],
            category="notification",
            description="系统化运维断点发生变化时推送 Discord；写 watchdog 状态文件，不运行 pipeline、不下单。",
            writes_state=True,
            writes_order=False,
            risk_level="notification_write",
            state_events=["ops_watchdog.state_file"],
            options={
                ...
                "--jobs-path": {"placeholder": "PATH", "required": False},
                "--env-file": {"placeholder": "PATH", "required": False},
                "--state-file": {"placeholder": "PATH", "required": False, "writes_filesystem": True},
                "--reset-state": {"type": "flag", "default": False},
                "--max-runtime-seconds": {"type": "int", "default": 45, "min": 5},
                "--dry-run": {"type": "flag", "default": False, "effect": "只生成卡片，不发送且不更新状态"},
                "--json": {"required_for_automation": True},
            },
        ),
